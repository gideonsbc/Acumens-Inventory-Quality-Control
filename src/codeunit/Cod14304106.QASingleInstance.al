codeunit 14304106 "QA Single Instance"
{
    SingleInstance = true;
    ...
    begin
        if Location.Get(LocationCode) then begin
            ShowQABin := Location."QA. Bin Restriction";
        end;
        if WhsJnlBatch.Get(JnlTemplateName, JnlBatchName, Location.Code) then if WhsJnlBatch."Allow QA. Transaction" then ShowQABin := false;
    end;
