codeunit 14304104 "QA Management"
{
    [TryFunction]
    ...
    var
        Item: Record Item;
        ItemRestrictionEntry: Record "Item Restriction Entry";
    ...
    begin
        WhseSetup.Get;
        WhseSetup.TestField("QA. Warehouse Batch Name");
        if Location.Get(LocationCode) then
            if Location."QA. Zone" <> '' then begin
    ...
    var
        QAWarehouseActivityLine: Record "QA-Warehouse Activity Line";
        ItemRestrictionEntry: Record "Item Restriction Entry";
        ItemRestrictions: Record "Item Restrictions";
        WhseSetup: Record "Warehouse Setup";
        ...
        FromBin: Record Bin;
        ToBin: Record Bin;
        WhseItemRestriction: Record "Warehouse Item Restriction";
        PostJnl: Boolean;
        RemQty: Decimal;
        DocNo: Code[20];
                    ...
                    repeat
                        ...
                        if WhseItemRestriction.FindFirst() then begin
                            ...
                            if TransferReceiptLine.FindFirst() then
                                if TransferReceiptLine."Restriction Code" <> '' then begin
                                    RestrictionCode := TransferReceiptLine."Restriction Code";
                                    RestrictionStatus := TransferReceiptLine."Restriction Status";
                                end;
                        end;
                ...
                if WhseItemRestriction.FindFirst() then begin
                    ...
                    if ItemRestrictionEntry.FindFirst() then begin
                        ...
                        if ItemRestrictionEntry."Remaining Qty." >= WarehouseEntry.Quantity then begin
                            ...
                            if FromBin."Restrict Item" then begin
    ...
    procedure RegisterRelease(var ItemRestrictions: Record "Item Restrictions")
    var
        ItemRestrictionEntry: Record "Item Restriction Entry";
        WhseSetup: Record "Warehouse Setup";
        Location: Record Location;
    ...
    [TryFunction]
    procedure RegisterScrap(var ItemRestrictions: Record "Item Restrictions")
    var
        ItemRestrictionEntry: Record "Item Restriction Entry";
        WhseSetup: Record "Warehouse Setup";
        Location: Record Location;
    ...
    procedure RegisterReclass(var ItemRestrictions: Record "Item Restrictions"; RestrictionCode: Code[20]; RestrictionStatus: Code[20]; FromBinCode: Code[20]; FBinCode: Code[20]; TBinCode: Code[20])
    var
        ItemRestrictionEntry: Record "Item Restriction Entry";
        WhseSetup: Record "Warehouse Setup";
        Location: Record Location;
    ...
    begin
        ...
        if Location."QA. Bin Restriction" then begin
            if not QASingleInstance.GetQARestriction() then if Bin.Get(WarehouseEntry."Location Code", WarehouseEntry."Bin Code") then if Bin."Zone Code" = Location."QA. Zone" then Location.TestField("QA. Bin Restriction", false);
        end;
        if Location."QA. Zone" <> '' then
                                ...
                                if Location.Get(WarehouseEntry."Location Code") then
                                    if Location."QA. Zone" <> '' then
                                        if Bin.Get(WarehouseEntry."Location Code", WarehouseEntry."Bin Code") then
                                            if Bin."Zone Code" = Location."QA. Zone" then begin
                                                WhseSetup.Get();
                                                WhseSetup.TestField("Inv. Counts Restriction Code");
                                                WhseSetup.TestField("Inv. Counts Restriction Status");
                                                RestrictionCode := WhseSetup."Inv. Counts Restriction Code";
                                                RestrictionStatus := WhseSetup."Inv. Counts Restriction Status";
    ...
    procedure ReleaseRestriction(var ItemRestrictions: Record "Item Restrictions"; DocNo: Code[20]; DocLineNo: Integer)
    var
        ItemRestrictionEntry: Record "Item Restriction Entry";
        Item: Record Item;
    ...
    begin
        ...
        if LotInfo.Get(ItemRestrictions."Item No.", ItemRestrictions."Variant Code", ItemRestrictions."Lot No.") then begin
            LotInfo."Restriction Code" := ItemRestrictions."Restriction Code";
            LotInfo."Restriction Status" := ItemRestrictions."Restriction Status";
            LotInfo.Modify();
        end;
    ...
    begin
        if Location.Get(LocationCode) then
            if Zone.Get(Location.Code, Location."QA. Zone") then begin
                Bin.SetRange("Location Code", Zone."Location Code");
                Bin.SetRange("Zone Code", Zone.Code);
                Bin.SetRange("QA. Bin", not (Zone."QA. Zone"));
                Bin.ModifyAll("QA. Bin", Zone."QA. Zone");
                BinContent.SetRange("Location Code", Zone."Location Code");
                BinContent.SetRange("Zone Code", Zone.Code);
                BinContent.SetRange("QA. Bin", not (Zone."QA. Zone"));
                BinContent.ModifyAll("QA. Bin", Zone."QA. Zone");
            end;
    end;
    ...
    begin
        WhseSetup.Get;
        WhseSetup.TestField("QA. Template Name");
        WhseSetup.TestField("QA. Batch Name");
        LineNo := 10000;
        ItemJnlLine.SetRange("Journal Template Name", WhseSetup."QA. Template Name");
        ItemJnlLine.SetRange("Journal Batch Name", WhseSetup."QA. Batch Name");
        if ItemJnlLine.FindLast() then LineNo := ItemJnlLine."Line No." + 10000;
        ItemJnlTemplate.Get(WhseSetup."QA. Template Name");
        ItemJnlBatch.Get(WhseSetup."QA. Template Name", WhseSetup."QA. Batch Name");
        ItemJnlLine.Init();
        ItemJnlLine."Journal Template Name" := WhseSetup."QA. Template Name";
        ItemJnlLine."Journal Batch Name" := WhseSetup."QA. Batch Name";
        ...
        ResEntry."Source ID" := WhseSetup."QA. Template Name";
    ...
    begin
        WhseSetup.Get;
        WhseSetup.TestField("QA. Warehouse Template Name");
        WhseSetup.TestField("QA. Warehouse Batch Name");
        WhsJnlLine.SetRange("Journal Template Name", WhseSetup."QA. Warehouse Template Name");
        WhsJnlLine.SetRange("Journal Batch Name", WhseSetup."QA. Warehouse Batch Name");
    ...
    begin
        ...
        WhsJnlTemplate.Get(WhseSetup."QA. Warehouse Template Name");
        WhsJnlBatch.Get(WhseSetup."QA. Warehouse Template Name", WhseSetup."QA. Warehouse Batch Name", LocationCode);
        WhsJnlLine.Init();
        WhsJnlLine."Journal Template Name" := WhseSetup."QA. Warehouse Template Name";
        WhsJnlLine."Journal Batch Name" := WhseSetup."QA. Warehouse Batch Name";
        ...
        if Bin.Get(LocationCode, BinCode) then begin
            ...
            if Location."QA. Zone" <> '' then
                if Bin."Zone Code" <> Location."QA. Zone" then begin
                    ...
                    QABin.SetRange("Zone Code", Location."QA. Zone");
                    QABin.SetRange(Code, Bin.Code + '-Q');
                    if not QABin.FindFirst() then begin
                        Zone.Get(LocationCode, Location."QA. Zone");
                        QABin.Init();
                        QABin.TransferFields(Bin);
                        QABin.Code := Bin.Code + '-Q';
                        QABin."Zone Code" := Location."QA. Zone";
    ...
    begin
        WarehouseSetup.Get();
        if WarehouseSetup."Allow DPP. Transfer" then
    ...
    begin
        ...
        if Location."QA. Zone" = BinContent."Zone Code" then
