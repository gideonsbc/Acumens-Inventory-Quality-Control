codeunit 14304105 "QA Event Subscriber"
{
        ...
        if not QASingleInstance.GetQABin() then
            if not WhseSetup."Allow Partial Release" then begin
                ...
                if WarehouseJournalLine.FindSet() then
                    repeat
                        IF (WarehouseJournalLine."Entry Type" = WarehouseJournalLine."Entry Type"::Movement) AND (WarehouseJournalLine."Journal Batch Name" = WhseSetup."QA. Warehouse Batch Name") and (WarehouseJournalLine."Journal Template Name" = WhseSetup."QA. Warehouse Template Name") then exit;
                                ...
                                if not Bin."Restrict Item" then begin
                                    if Location."Adjustment Bin Code" <> WarehouseJournalLine."From Bin Code" then
                                        if QABin.Get(WarehouseJournalLine."Location Code", WarehouseJournalLine."From Bin Code" + '-Q') then begin
                                            WarehouseJournalLine."From Zone Code" := Location."QA. Zone";
                                    ...
                                    if Location."Adjustment Bin Code" <> WarehouseJournalLine."To Bin Code" then
                                        if WarehouseJournalLine."To Zone Code" <> Location."QA. Zone" then begin
                                            WarehouseJournalLine."To Zone Code" := Location."QA. Zone";
        ...
        if WhseJrnlLine.FindSet() then
            repeat
                if WhsJnlBatch.Get(WhseJrnlLine."Journal Template Name", WhseJrnlLine."Journal Batch Name", WhseJrnlLine."Location Code") then
                    if WhsJnlBatch."Restrict Item" then
    ...
    begin
        ...
        if Location.Get(LocationCode) then if Location."QA. Bin Restriction" then if QASingleInstance.GetShowQABin() then Bin.SetRange("QA. Bin", false);
    end;
    ...
    begin
        if Location.Get(LocationCode) then
            if Location."QA. Bin Restriction" then begin
                ...
                if QASingleInstance.GetShowQABin() then BinContent.SetRange("QA. Bin", false);
    ...
    begin
        WarehouseSetup.Get();
        if not WarehouseSetup."Allow DPP. Transfer" then exit;
    ...
    var
        ...
        QASingleInstance: Codeunit "QA Single Instance";
    begin
        if WhsJnlBatch.Get(WarehouseJournalLine."Journal Template Name", WarehouseJournalLine."Journal Batch Name", WarehouseJournalLine."Location Code") then QASingleInstance.SetQARestriction(WhsJnlBatch."Allow QA. Transaction");
    end;
    ...
    begin
        if TransLine."Restriction Code" <> '' then begin
            TransRcptLine."Restriction Code" := TransLine."Restriction Code";
            TransRcptLine."Restriction Status" := TransLine."Restriction Status";
        end;
    end;
    ...
    begin
        if (WhseJnlLine."Source Type" = Database::"Transfer Line") and (WhseJnlLine."Source Subtype" = 0) then begin
            ...
            if FromLocation.Get(WhseJnlLine."Location Code") then
                if FromLocation."QA. Zone" = WhseJnlLine."From Zone Code" then
                    if ToLocation.Get(TransferLine."Transfer-to Code") then
                        if ToLocation."QA. Zone" <> '' then begin
                            ...
                            if ItemRest.FindFirst() then begin
                                TransferLine."Restriction Code" := ItemRest."Restriction Code";
                                TransferLine."Restriction Status" := ItemRest."Restriction Status";
                                TransferLine.Modify();
    ...
    begin
        if (WarehouseJournalLine."Source Type" = Database::"Transfer Line") and (WarehouseJournalLine."Source Subtype" = 0) then begin
            if TransferLine.Get(WarehouseJournalLine."Source No.", WarehouseJournalLine."Source Line No.") then begin
                if ToLocation.Get(TransferLine."Transfer-to Code") then if FromLocation.Get(WarehouseJournalLine."Location Code") then if FromLocation."QA. Zone" = WarehouseJournalLine."From Zone Code" then if ToLocation."QA. Zone" <> '' then QASingleInstance.SetQARestriction(false);
            end;
        end;
    end;
                    ...
                    if WarehouseActivityLine."Action Type" = WarehouseActivityLine."Action Type"::Take then
                        if (WarehouseActivityLine."Source Type" = Database::"Transfer Line") and (WarehouseActivityLine."Source Subtype" = 0) then begin
                            if TransferLine.Get(WarehouseActivityLine."Source No.", WarehouseActivityLine."Source Line No.") then begin
                                ...
                                if FromLocation.Get(WarehouseActivityLine."Location Code") then if FromLocation."QA. Zone" = WarehouseActivityLine."Zone Code" then if ToLocation."QA. Zone" <> '' then QASingleInstance.UnBlockLot(WarehouseActivityLine."Item No.", WarehouseActivityLine."Variant Code", WarehouseActivityLine."Lot No.");
                            end;
                        end;
    end;
                            ...
                            if (ItemJournalLine."New Location Code" <> ItemJournalLine."Location Code") then begin
                                ...
                                if Rec."Location Code" = ItemJournalLine."Location Code" then begin
                                    if FLocation."Directed Put-away and Pick" then begin
                                        ...
                                        if FLocation."QA. Zone" <> '' then begin
                                            FBin.Get(ItemJournalLine."Location Code", ItemJournalLine."Bin Code");
                                            TBin.Get(ItemJournalLine."New Location Code", ItemJournalLine."New Bin Code");
                                            if FBin."Zone Code" = FLocation."QA. Zone" then begin
                                                if (FLocation."QA. Zone" <> '') <> (TLocation."QA. Zone" <> '') then Error(QAToQAError);
                                                if TBin."Zone Code" = TLocation."QA. Zone" then begin
                                                    if (FLocation."QA. Zone" <> '') <> (TLocation."QA. Zone" <> '') then Error(QAToQAError);
                                                end
                                                else
                                                    Error(QAToQAError);
                                                ItemJnlBatch.TestField("Allow QA. Transaction");
                                                FLocation.TestField("Allow QA. Transfer");
                                                TLocation.TestField("Allow QA. Transfer");
                                                QASingleInstance.SetQARestriction(true);
                                ...
                                if Rec."Bin Code" = WhseJournalLine."From Bin Code" then begin
                                    if FLocation."Directed Put-away and Pick" then begin
                                        if FLocation."QA. Zone" <> '' then begin
                                            ...
                                            if FBin."Zone Code" = FLocation."QA. Zone" then begin
                                                WhseJnlBatch.TestField("Allow QA. Transaction");
                                                QASingleInstance.SetQARestriction(true);
            ...
            if not WhseSetup."Allow Partial Release" then begin
                ...
                if ItemRestrictions.FindFirst() then
                    if WhseJnlLine.Get(Rec."Source Batch Name", Rec."Source ID", Rec."Location Code", Rec."Source Ref. No.") then begin
                        IF (WhseJnlLine."Entry Type" = WhseJnlLine."Entry Type"::Movement) AND (WhseJnlLine."Journal Batch Name" = WhseSetup."QA. Warehouse Batch Name") and (WhseJnlLine."Journal Template Name" = WhseSetup."QA. Warehouse Template Name") then exit;
    ...
    begin
        ...
        if ItemRestrictionEntry.ReadPermission() then begin
            ...
            Navigate.InsertIntoDocEntry(DocumentEntry, Database::"Item Restriction Entry", ItemRestrictionEntry.TableCaption(), ItemRestrictionEntry.Count);
        end;
    end;
    ...
    begin
        ...
        case TableID of
            Database::"Item Restriction Entry":
                begin
